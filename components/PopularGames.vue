                    ...
                    <div class="col-span-full lg:col-span-6">
                        ...
                        <div v-for="content in brandContent" :key="content.id">
                            <p class="info_content text-[#bacfdc] font-light text-lg py-5 px-4">
                                {{ content.acf.popular_games_info }}
                            </p>
                        </div>
                    </div>
...
const cacheKey = computed(() => getCacheKey('popular-games', {
  lang: lang.value,
...
onMounted(async () => {
  try {
    ...
    if (cachedGames) {
      popularGames.value = cachedGames;
      loading.value = false;
      return;
    }
    ...
    // Fetch fresh data
    await fetchGames();
    setCache(cacheKey.value, popularGames.value);
  } catch (error) {
    console.error('Error in Popular Games:', error);
  } finally {
    loading.value = false;
  }
});
